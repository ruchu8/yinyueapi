<?php
/**************************************************
 * 网易云音乐ID转播放地址/封面（302重定向版）
 * 功能：传入网易云音乐ID，直接重定向到真实MP3地址或封面图片
 *************************************************/
...
// 2. 引入精简后的Meting核心类（保留网易云播放地址和封面获取逻辑）
class Meting {
    public $raw;
    public $info;
    public $error;
    ...
    public function pic($id, $size = 300) {
        $encryptedId = $this->netease_encryptId($id);
        return "https://p3.music.126.net/{$encryptedId}/{$id}.jpg?param={$size}y{$size}";
    }
...
// 3. 核心业务逻辑：根据参数类型（id/pic）执行对应重定向
use Metowolf\Meting;
$api = new Meting();
...
if (isset($_GET['pic'])) {
    $picId = trim($_GET['pic']);
    // 验证封面ID有效性
    if (empty($picId) || !is_numeric($picId) || strlen($picId) < 8 || strlen($picId) > 20) {
        http_response_code(400);
        echo "无效的封面ID！请在URL后添加 ?pic=网易云封面ID";
        exit;
    }
    // 获取尺寸参数（默认300，限制100-1000）
    $size = isset($_GET['size']) ? (int)$_GET['size'] : 300;
    $size = max(100, min(1000, $size));
    // 生成封面地址并跳转
    $coverUrl = $api->pic($picId, $size);
